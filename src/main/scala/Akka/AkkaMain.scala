...
import Akka.Dispatcher.task_dispatcher.LogWorker.Log
...
object Dispatcher extends App {
  object task_dispatcher {
    ...
    def apply(): Behavior[TaskDispetcherProtocol] = Behaviors.setup { ctx =>
      ...
      Behaviors.receiveMessage {
        case ParseUrl(url) =>
          val ref = ctx.spawn(ParseUrlWorker(), s"ParseWorker")
          ref ! ParseUrlWorker.ParseUrl(url, adapter2)
          Behaviors.same
        case Log(str) =>
          val ref = ctx.spawn(LogWorker(), "ajdakjsdf")
  ...
  def apply(): Behavior[NotUsed] =
    Behaviors.setup { ctx =>
      val disp = ctx.spawn(task_dispatcher(), "disp.")
      disp ! task_dispatcher.Log("111111")
      disp ! task_dispatcher.ParseUrl("222222")
      Behaviors.same
    }

  implicit val system = ActorSystem(Dispatcher(), "dsasdfsdf")

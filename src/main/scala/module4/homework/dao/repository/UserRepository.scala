...
object UserRepository{
    ...
    class ServiceImpl extends Service{
        ...
        def createUsers(users: List[User]): Result[List[User]] = run {
            liftQuery(users).foreach (user => userSchema.insert(user))
        } zipRight list()
        ...
        def userRoles(userId: UserId): Result[List[Role]] = run {
            userSchema.filter(_.id == lift(userId.id)).flatMap { user =>
                userToRoleSchema.filter(_.userId == user.id).flatMap { role =>
                    roleSchema.filter(_.code == role.roleId)
                }
            }
        }

        def insertRoleToUser(roleCode: RoleCode, userId: UserId): ZIO[Environment, Error, Unit] = {
        ...
        def listUsersWithRole(roleCode: RoleCode): Result[List[User]] = run {
            roleSchema.filter(_.code == lift(roleCode.code)).flatMap { role =>
                userToRoleSchema.filter(_.roleId == role.code).flatMap { user =>
                    userSchema.filter(_.id == user.userId)
                }
            }
        }
        ...
        def findRoleByCode(roleCode: RoleCode): Result[Option[Role]] =
            run(roleSchema.filter(_.code == lift(roleCode.code))
              .sortBy(_.code).take(1))
              .map(_.headOption)

        override def createRole(role: Role): QIO[Role] = run(roleSchema.insert(lift(role))).as(role)
    }
